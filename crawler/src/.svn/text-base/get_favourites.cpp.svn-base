...
//        printf( "\nUsing:\nKey: %s\nSecret: %s\n\n", myOAuthAccessTokenKey.c_str(), myOAuthAccessTokenSecret.c_str() );
    ...
    string event("start_get_favourites "), description("");
    event += (string)start_time;
    db.log(event, description);
    string order_by = "last_get_favourites, distance_to_seed";
    ...
    int i;
    string  statuses(""), fields("last_favourite"),
    		fields_status("of_user_id, id, user_id, created_at, text, source, truncated, in_reply_to_status_id, in_reply_to_user_id, retweet_count, geo, coordinates, place, contributors, favorited, retweeted, retweeted_status, retweeted_user_id, possibly_sensitive, user_mentions, hashtags, urls");
    unsigned long long last_id, since_id;
    while(iss>>user_id){
  	  cout<<"updating favourites of the user "<<user_id<<endl;
	  ...
	  iss1>>since_id;
  	  ...
  	  {
  		  last_id = 0;
  		  if(!twitterObj.getUserFavourites(statuses,  last_id, fields_status,  since_id, user_id, true)){
  	  ...
  	  if(i == crawl_max_retry){
  	    		  ...
  	    		  fields_update = "last_get_favourites"; values_update = "CURRENT_TIMESTAMP";
	  ...
	  if(db.insertFavourites(fields_status, statuses)){
	    cout<<"inserted favourites for the user "<<user_id<<endl;
	  }
	  fields_update = "last_get_favourites, last_favourite";
	  values_update = "CURRENT_TIMESTAMP, "+boost::lexical_cast<string>(last_id);
	  if(!db.updateUser(user_id, fields_update, values_update))
	  {
		  cout <<"error update user's favourites"<<endl;
	  }

    }

    event = "finish_get_favourites_started_at "+(string)start_time;
    db.log(event, description);
    db.disconnect();
