...
bool twitCurl::geoSearch( std::string& searchQuery )
{
    /* Prepare URL */
    std::string buildUrl = twitterDefaults::TWITCURL_GEO_SEARCH_URL +
                           twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[twitCurlTypes::eTwitCurlApiFormatJson] +
                           twitCurlDefaults::TWITCURL_URL_SEP_QUES + twitCurlDefaults::TWITCURL_GEOSEARCHQUERYSTRING +
                           urlencode(searchQuery);

    /* Perform GET */
    return performGet( buildUrl );
}
bool twitCurl::search( std::string& searchQuery, unsigned long long since_id, unsigned long long max_id)
{
    /* Prepare URL */
    std::string buildUrl = twitterDefaults::TWITCURL_SEARCH_URL +
                           twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[twitCurlTypes::eTwitCurlApiFormatJson] +
    ...
    if(since_id)
        {
            ...
            tmpStrm << twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_SINCE_ID << since_id;
            buildUrl += tmpStrm.str();
            tmpStrm.str().clear();
        }
    if(max_id){
            std::stringstream tmpStrm;
            tmpStrm << twitCurlDefaults::TWITCURL_URL_SEP_AMP + twitCurlDefaults::TWITCURL_MAX_ID << max_id;
            buildUrl += tmpStrm.str();
            tmpStrm.str().clear();
     }
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"rpp=100";
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"result_type=recent";
    /* Perform GET */
    return performGet( buildUrl );
}
bool twitCurl::search( std::string& searchQuery, unsigned long long since_id, unsigned long long max_id, std::string & lang)
{
    /* Prepare URL */
    std::string buildUrl = twitterDefaults::TWITCURL_SEARCH_URL +
                           twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[twitCurlTypes::eTwitCurlApiFormatJson] +
    ...
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"rpp=100";
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"result_type=recent";
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"lang="+lang;
...
bool twitCurl::timelineUserGet( bool trimUser /* in */, bool includeRetweets /* in */, unsigned int tweetCount /* in */, unsigned long long since_id, unsigned long long int max_id, std::string userInfo /* in */, bool isUserId ){
    /* Prepare URL */
    std::string buildUrl;
    ...
    /* Perform GET */
    buildUrl += twitCurlDefaults::TWITCURL_URL_SEP_AMP+"include_entities=true";
    return performGet( buildUrl );
	return true;
}
...
/*++
* @method: twitCurl::usersGet
*
* @description: method to get many users' profiles
*
* @input: usersInfo - list of screen names or user ids in string format, seperated by comma,
*         isUserId - true if userInfo contains an id
*
* @output: true if GET is success, otherwise false. This does not check http
*          response by twitter. Use getLastWebResponse() for that.
*
*--*/
bool twitCurl::usersGet( std::string& usersInfo, bool isUserId )
{
    bool retVal = false;
    if( usersInfo.length() )
    {
        /* Set URL */
        std::string buildUrl;
        utilMakeUrlForUser( buildUrl, twitterDefaults::TWITCURL_LOOKUPUSERS_URL +
                            twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType],
                            usersInfo, isUserId );
...
bool twitCurl::friendsIdsGet( std::string& userInfo, std::string& cursor, bool isUserId )
{
    /* Prepare URL */
    std::string buildUrl;
    utilMakeUrlForUser( buildUrl, twitterDefaults::TWITCURL_FRIENDSIDS_URL +
...
*
*--*/
bool twitCurl::followersIdsGet( std::string& userInfo, std::string& cursor, bool isUserId )
{
    /* Prepare URL */
    std::string buildUrl;
    utilMakeUrlForUser( buildUrl, twitterDefaults::TWITCURL_FOLLOWERSIDS_URL +
...
bool twitCurl::favoritesGet( bool trimUser /* in */, bool includeRetweets /* in */, unsigned int tweetCount /* in */, unsigned long long since_id, unsigned long long int max_id, std::string userInfo /* in */, bool isUserId ){
    /* Prepare URL */
    std::string buildUrl;

    utilMakeUrlForUser( buildUrl, twitterDefaults::TWITCURL_FAVORITESGET_URL +
                        twitCurlDefaults::TWITCURL_EXTENSIONFORMATS[m_eApiFormatType],
                        userInfo, isUserId );
